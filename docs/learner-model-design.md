...
- **The model now says when its ordering is noise.** `LearnerModel.separation(student)` returns the Rasch separation index over the label *deviations* (`G = true SD / RMSE`, reliability `G²/(1+G²)`, strata `(4G+1)/3`). `distinguishable` is `G ≥ 2` — reliability 0.8, about three strata, the usual bar for a ranking one acts on. The formula assumes unshrunk measures, so each label's posterior is first de-shrunk by the estimator's own `label_prior_var` (error variance `s² = 1/(1/var − 1/label_prior_var)`, measure `mu·s²/var`). Fed the shrunk posteriors directly it reads about `G² − 1` in place of `G²` even with a correct prior, and far less at moderate data — separable labels read as noise (found in the post-merge review of #10; fixed in #11).
